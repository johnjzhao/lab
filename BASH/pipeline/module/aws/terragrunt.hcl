...
terraform {
  extra_arguments "publish_vars" {
    ...
    arguments = [
      ...
      "-var", "tfstate_bucket=svci-tf-state-${get_env("TF_VAR_account_number", "<DEFAULT_ACCOUNT_NUMBER>")}",
    ...
    required_var_files = [
      ...
      "${get_terragrunt_dir()}/env-config/${get_env("TF_VAR_env", "local")}.tfvars"
    ]
  }
}
...
remote_state {
  ...
  config = {
    bucket         = "svci-tf-state-${get_env("TF_VAR_account_number", "<DEFAULT_ACCOUNT_NUMBER>")}"
    dynamodb_table = "svci-tf-lock-${get_env("TF_VAR_account_number", "<DEFAULT_ACCOUNT_NUMBER>")}"
