...
module "pcs_error_report_extract_glue_job" {
  source = "./modules/glue"

  cloudwatch_log_group_name = "/aws-glue/jobs/${var.gov_solutions_commissions_tdv_to_s3_pcs_error_report_job}"
  retention_in_days         = 14

  name                = var.gov_solutions_commissions_tdv_to_s3_pcs_error_report_job
  description         = "Pcs error report extract glue job"
  ...
  script_location     = "s3://${var.artifacts_bucket}/${var.artifacts_bucket_script_key}/${var.gov_solutions_commissions_tdv_to_s3_pcs_error_report_path}"
  ...
  arguments = tomap(
    {
      "--scriptLocation"           = "s3://${var.artifacts_bucket}/${var.artifacts_bucket_script_key}/${var.gov_solutions_commissions_tdv_to_s3_pcs_error_report_path}"
      ...
      "--custom_log_group"         = "/aws-glue/jobs/${var.gov_solutions_commissions_tdv_to_s3_pcs_error_report_job}"
...
resource "aws_glue_crawler" "pcs_error_report_crawler" {
  database_name = var.aws_glue_catalog_database.name
  name          = "gov_solutions_commissions_pcs_error_report_crawler"
  role          = var.glue_role_arn
  s3_target {
    path = "s3://${var.artifacts_bucket}/${var.artifacts_bucket_key}/extracts/src_data_dim/"
  }
