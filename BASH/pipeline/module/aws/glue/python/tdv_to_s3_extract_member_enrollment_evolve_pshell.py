...
logger = ScriptLogger("tdv_to_s3_dataextract_membere_evolve").get_logger(action = ACTION)
...
mem_dim_fileloc = sql_props_location + '/tdv_extracts/evolve_membere_tdv_to_s3.properties'
...
q_SELECT_VT_MEME_DATA = props.get("q_SELECT_VT_MEME_DATA").data.format(DATAMART_MEMBER=schema_datamart_member, REPORTING=schema_reporting, TRXHUB_CORE=trxhub_core)
logger.info(f"q_SELECT_VT_MEME_DATA: {q_SELECT_VT_MEME_DATA}")
...
def executeStmtOnTDV():
    ...
    try:
        ...
        src_df = pd.read_sql(q_SELECT_VT_MEME_DATA, conn)
        conn.close()
        return src_df
    except:
        logger.info(f"Failed execution of the queries to generate the enrollment parquet file.")
