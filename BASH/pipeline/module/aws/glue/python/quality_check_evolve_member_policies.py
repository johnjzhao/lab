...
def main():
    try:
        ...
        read_filepath = "s3://gov-solutions-commissions-artifacts-dev/artifactory/glue/extracts/evolve/" \
                        "memberp_evolve/memp_test.txt"
        print(f"s3_readPath for 'evolve_member_policies' : {read_filepath}")
        write_filepath = "s3://gov-solutions-commissions-artifacts-dev/artifactory/glue/quality_results/evolve"
        print(f"s3_writePath for 'evolve_member_policies' : {write_filepath}")
        ...
        write_dataframe_to_parquet_on_s3(general_stats_df, write_filepath + "/evolve_memp_quality_general_stats"
                                         + "/" + curr_datetime, "test_date")
        print(f"general_stats_f written")

        # specify expected column datatypes in a list []
        # Options are "date", "string", "number", "number and letter", "phone"
        col_datatypes = ["number_letter", "number_letter", "number_letter", "number_letter", "number", "date_evolve",
                         "gender", "phone", "number_letter", "number_letter", "number_letter", "number_letter",
                         "number", "date_evolve"]
        ...
        # write dataframes to s3
        write_dataframe_to_parquet_on_s3(formats_df, write_filepath + "/evolve_memp_quality_formats" + "/"
                                         + curr_datetime, "test_date")
        print(f"formats_df written")
        print("main_executed")

    except Exception as error:
        logging.exception("Error in Main Execution (quality_check_evolve_member_policies)")
