...
logger = ScriptLogger("tdv_to_s3_dataextract_mem_dim").get_logger(action = ACTION)
...
args = getResolvedOptions(sys.argv, ['region_name', 'host', 'artifacts_bucket', 'tdv_env', 'sqlfile', 'artifacts_bucket_key', 'username', 'tdv_secretsmanager', 'custom_log_group', 'schema_datamart_member'])

datamart_member = args['schema_datamart_member']
...
mem_dim_fileloc = sql_props_location + '/tdv_extracts/mem_dim_sql.properties'
...
q_CREATE_VT_MBR_DIM = props.get("q_CREATE_VT_MBR_DIM").data.format(DATAMART_MEMBER=datamart_member)
logger.info(f"q_CREATE_VT_MBR_DIM: {q_CREATE_VT_MBR_DIM}")

q_SELECT_VT_MBR_DIM = props.get("q_SELECT_VT_MBR_DIM").data
logger.info(f"q_SELECT_VT_MBR_DIM: {q_SELECT_VT_MBR_DIM}")
...
def executeStmtOnTDV():
    ...
    try:
        ...
        cur.execute(q_CREATE_VT_MBR_DIM)
        src_df = pd.read_sql(q_SELECT_VT_MBR_DIM, conn)
        conn.close()
        return src_df
    except:
        cur.execute(q_CREATE_VT_MBR_DIM)
