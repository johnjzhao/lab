...
def main():
    try:
        ...
        read_filepath = "s3://" + s3_bucket + "/" + artifacts_bucket_key + \
                        "/transform/pcs/pcs_weekly_agent_enrollment_transactions_report/part-00000-13bc2209-" \
                        "565b-44c4-8f79-1bbfb1eb7abf-c000.csv"
        print(f"s3_readPath for 'pcs_agent_enrollment_transactions' : {read_filepath}")
        write_filepath = "s3://" + s3_bucket + "/" + artifacts_bucket_key + "/quality_results/pcs"
        print(f"s3_writePath for 'pcs_agent_enrollment_transactions' : {write_filepath}")

        # read file
        pcs_df = spark.read.option("delimiter", ",").option("header", True).csv(read_filepath)
        ...
        general_stats_df = QualityMethods.quality_general_stats(pcs_df, curr_datetime)
        print(f"general_stats eval complete")

        # write dataframes to s3
        write_dataframe_to_parquet_on_s3(general_stats_df, write_filepath + "/pcs_weekly_enr_quality_general_stats"
        ...
        col_datatypes = ["src_member", "number", "date", "date_pcs", "date_pcs", "date_pcs", "number",
                         "number and letter"]
        ...
        formats_df = QualityMethods.quality_formats(pcs_df, col_datatypes,  curr_datetime)
        print(f"formats eval complete")

        # write dataframes to s3
        write_dataframe_to_parquet_on_s3(formats_df, write_filepath + "/pcs_weekly_enr_quality_formats" + "/"
                                         + curr_datetime, "test_date")
