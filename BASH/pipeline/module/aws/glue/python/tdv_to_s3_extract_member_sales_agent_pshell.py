...
args = getResolvedOptions(sys.argv, ['region_name', 'host', 'artifacts_bucket', 'tdv_env', 'sqlfile', 'artifacts_bucket_key', 'username', 'tdv_secretsmanager', 'custom_log_group', 'schema_trxhub'])
...
mem_sales_agent_fileloc = sql_props_location + '/tdv_extracts/mem_sales_agent_sql.properties'
...
#######################
response = s3_client.get_object(Bucket=s3_bucket, Key=mem_sales_agent_fileloc)
...
q_SELECT_MEM_SALES_AGENT = props.get("q_SELECT_MEM_SALES_AGENT").data.format(TRXHUB_CORE=schema_trxhub)
print(f"q_SELECT_MEM_SALES_AGENT: {q_SELECT_MEM_SALES_AGENT}")

writePath_MEM_SALES_AGENT = props.get("s3_writePath_MEM_SALES_AGENT").data
print(f"s3_writePath for 'CDO_MBR_DEMG' : {writePath_MEM_SALES_AGENT}")
...
def executeStmtOnTDV():
    ...
    try:
        ...
        final_df = pd.read_sql(q_SELECT_MEM_SALES_AGENT, conn)
...
def extractFromTDVToS3():
    try:
        ...
        df_MBR_SALES_AGENT = executeStmtOnTDV()
       
        # Fetch current date and time
        now = datetime.now()
        curr_datetime = now.strftime("%d%m%Y %H%M%S")
        
        s3_write_path_MBR_SALES_AGENT = writePath_MEM_SALES_AGENT + curr_datetime + '.parquet'
        
        write_dataframe_to_parquet_on_s3(df_MBR_SALES_AGENT,s3_write_path_MBR_SALES_AGENT)
    except Exception as error:
...
def main():
    ...
    try:
        ...
        extractFromTDVToS3() 
        logger.info("Starting extraction: PCS Member Sales Agent")
    except Exception as error:
