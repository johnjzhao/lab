...
function set_and_fed() {
    ...
    TERRAGRUNT_VERSION="${TERRAGRUNT_VERSION:-0.37.4}"
    ...
    if ! terraform -v | grep "v${TERRAFORM_VERSION}"; then
      if tfswitch "${TERRAFORM_VERSION}" > /dev/null; then
        echo "Current Terraform version is now $(terraform -v)"
      else
        echo "Problem with tfswitch. Downloading terraform ${TERRAFORM_VERSION} from hashicorp release directly"
        curl -s -qL -o /tmp/terraform.zip "https://releases.hashicorp.com/terraform/${TERRAFORM_VERSION}/terraform_${TERRAFORM_VERSION}_linux_amd64.zip"
        unzip -qo /tmp/terraform.zip -d /home/jenkins/.bin
