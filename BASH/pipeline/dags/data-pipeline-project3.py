...
dag_id = f'{REPO_NAME}-project3' # ALL DAG_IDs MUST begin with the REPO NAME, i.e. if repo name is 'tdv-oss-demo', dag_id = 'tdv-oss-demo-create-glue-jobs'
...
with DAG(dag_id = dag_id, default_args = default_args, schedule_interval = None) as dag:
    ...
    claims1_sql = CustomGlueJobOperator(
            ...
            script_args = get_glue_params( 
                {
                        "--extra-py-files": f"s3://{TDV_GLOBAL_CONFIG['glue_s3_bucket']}/{REPO_NAME}/dml/project3/claims1_sql.py", # python file name
    ...
    claims2_sql = CustomGlueJobOperator(
            ...
            script_args = get_glue_params( 
                {
                        "--extra-py-files": f"s3://{TDV_GLOBAL_CONFIG['glue_s3_bucket']}/{REPO_NAME}/dml/project3/claims2_sql.py", # python file name
    ...
    claims3_sql = CustomGlueJobOperator(
            ...
            script_args = get_glue_params( 
                {
                        "--extra-py-files": f"s3://{TDV_GLOBAL_CONFIG['glue_s3_bucket']}/{REPO_NAME}/dml/project3/claims3_sql.py", # python file name
